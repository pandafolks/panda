...
trait ParticipantEventDao {
  ...
  /** Checks whether participant with requested identifier exists. If there was a Removed item emitted and there was no
    ...
    * @param participantEventOperator
    *   Participant DB entry point
    * @return
    *   True if exists, false if not or [[PersistenceError]] if the query could not be executed
    */
  def exists(
      identifier: String,
  ...
  /** Inserts participant event into the persistence layer.
    *
    * @param participantEvent
    *   Participant event represented as an object of common type for all types of events
    * @param participantEventOperator
    *   Participant DB entry point
    * @return
    *   Either empty if saved successfully or [[PersistenceError]] if the error during saving occurred
    */
  def insertOne(
  ...
  /** Return stream of all available events ordered by event IDs with the IDs higher than offset.
    ...
    *   Participant DB entry point
    * @param offset
    *   Describes maximum discarded event identifier
    * @return
    *   Stream of the participant events
    */
  def getOrderedEvents(
      participantEventOperator: CollectionOperator[ParticipantEvent],
      offset: Int = -1
