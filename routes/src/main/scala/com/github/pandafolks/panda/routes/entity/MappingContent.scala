...
object MappingContent {
  def fromMappingPayload(mapping: MappingPayload): MappingContent = {
    def rc(mapping: MappingPayload, initial: Boolean): MappingContent =
      mapping.value match {
        case Left(v) => MappingContent(left = Some(if (initial) v else unifyFromSlashes(v)), right = Option.empty) // Group may contain leading or ending slashes. However, all routes have to be escaped.
