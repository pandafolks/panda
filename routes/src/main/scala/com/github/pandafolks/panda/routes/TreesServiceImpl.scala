...
final class TreesServiceImpl private (
  ...
  locally {
    if (treesHandlerRefreshIntervalInMillis > 0) {
      ...
      backgroundJobsRegistry.addJobAtFixedRate(
        treesHandlerRefreshIntervalInMillis.millisecond,
        treesHandlerRefreshIntervalInMillis.millisecond
      )(
        () =>
          reloadTreesIfNecessary()
            .onErrorRecover { e: Throwable =>
              logger.error(s"Cannot reload ${RoutesTreesHandler.getClass.getName}", e)
            },
  ...
  private def reloadTreesIfNecessary(): Task[Unit] =
    Task.eval(logger.debug("Starting TreesServiceImpl#reloadTreesIfNecessary job")) >>
      c.use { case (mapperOperator, prefixOperator) =>
        for {
  ...
  def apply(
    ...
    c.use { case (mapperOperator, prefixOperator) =>
      for {
        ...
        routesTreesHandler,
        latestSeenMappingTimestamp,
        latestSeenPrefixTimestamp
      )(c)
    }
