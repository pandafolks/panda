...
final class MapperDaoImpl extends MapperDao {
  ...
  override def delete(route: String, method: Option[String])(mapperOperator: CollectionOperator[Mapper]): Task[Either[PersistenceError, String]] = {
    ...
    val unifiedRoute = unifyFromSlashes(route)
    mapperOperator.single.deleteMany(getUniqueMapperFilter(unifiedRoute, unifiedHttpMethod))
      .map { deleteRes =>
        if (deleteRes.deleteCount > 0) Right(unifiedRoute) else Left(NotExists(s"Route \'$route\' [${unifiedHttpMethod.getName}] does not exist"))
      }
