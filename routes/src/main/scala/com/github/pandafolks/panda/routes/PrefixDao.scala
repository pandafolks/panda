...
trait PrefixDao {
  ...
  /** Saves in the persistence layer the [[Prefix]] with the requested group name and prefix if the one for the
    ...
    *   [[Prefix]] DB entry point
    *
    * @return
    *   Either group name if saved successfully or PersistenceError if the error during saving occurred
    */
  def savePrefix(groupName: String, prefix: String)(
  ...
  /** Saves in the persistence layer the [[Prefix]] with the requested group name and prefix if the one for the
    ...
    *   Either group name if saved successfully or PersistenceError if the error during saving occurred
    */
  def saveOrUpdatePrefix(groupName: String, prefix: String)(
      prefixOperator: CollectionOperator[Prefix]
  ): Task[Either[PersistenceError, String]]
  ...
  /** Removes [[Prefix]] associated with requested group name.
    ...
    *   [[Prefix]] DB entry point
    * @return
    *   Either group name if deleted successfully or PersistenceError if the error during deletion occurred
    */
  ...
  /** Checks whether there is at least one [[Prefix]] with the lastUpdateTimestamp strictly higher than provided value
    ...
    *   [[Prefix]] DB entry point
    * @return
    *   True if there are prefix with higher lastUpdateTimestamp, false otherwise
    */
