...
final class PrefixDaoImpl extends PrefixDao {
  ...
  override def delete(groupName: String)(prefixOperator: CollectionOperator[Prefix]): Task[Either[PersistenceError, String]] =
    prefixOperator.single.deleteMany(Filters.eq(GROUP_NAME_PROPERTY_NAME, groupName.trim))
      .map { deleteRes =>
        if (deleteRes.deleteCount > 0) Right(groupName.trim) else Left(NotExists(s"There is no prefix associated with the group \'${groupName.trim}\'"))
      }
      .onErrorRecoverWith { case t: Throwable => Task.now(Left(UnsuccessfulDeleteOperation(t.getMessage))) }

  override def checkIfThereAreNewerPrefixes(timeStamp: Long)(prefixOperator: CollectionOperator[Prefix]): Task[Boolean] =
    prefixOperator.source.aggregate(
      List(
