...
class RoutesTreeImplTest extends AnyFlatSpec {
  ...
  private val commonData = List(
    ...
    Mapper("planes/{{plane_id}}/passengers", MappingContent(Some("planes"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
  ...
  it should "put the Wildcard at the end of the children" in {
    val data = List(
      ...
      Mapper("{{some_wildcard}}", MappingContent(Some("cars"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
  ...
  it should "should handle Pocket while favoring Fixed paths and Wildcards" in {
    val tree: RoutesTree = RoutesTreeImpl.construct(List(
      Mapper("supercars/fixed/", MappingContent(Some("fixedGroup"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
      Mapper("supercars/**", MappingContent(Some("pocketGroup"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
      Mapper("supercars/blabla/{{blabla_id}}", MappingContent(Some("wildcardGroup"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
    ...
    it should "return be able to return multiple Wildcard mappings" in {
      val tree: RoutesTree = RoutesTreeImpl.construct(List(
        Mapper("supercars/blabla/{{first_wildcard}}", MappingContent(Some("wildcardGroup1"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
        Mapper("supercars/blabla/{{first_wildcard}}/someFixed", MappingContent(Some("wildcardGroup2"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
        Mapper("supercars/blabla/{{first_wildcard}}/{{second_wildcard}}/blabla2/{{third_wildcard}}", MappingContent(Some("wildcardGroup3"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
        Mapper("supercars/fixed/fixed2", MappingContent(Some("fixedGroup2"), Option.empty), HttpMethod.Get(), isStandalone = true, -1),
      ))
