...
class SequenceDaoImplItTest extends AsyncFlatSpec with Matchers with ScalaFutures
  ...
  "SequenceDaoImpl#getNextSequence" should "generate unique sequence values in a multi-threading environment" in {
    ...
    val results: CancelableFuture[List[Either[PersistenceError, BsonInt64]]] = sequenceConnection.use(c =>
      Task.parTraverseUnordered(1 to 10000)(_ => sequenceDao.getNextSequence(sequenceKey, c))
  ...
  it should "be able to handle multiple sequence keys" in {
    ...
    val results: CancelableFuture[(List[Either[PersistenceError, BsonInt64]], List[Either[PersistenceError, BsonInt64]], List[Either[PersistenceError, BsonInt64]])] =
      sequenceConnection.use(c =>
        Task.parZip3(
